  ...
  return (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
       ...
       flex-wrap md:mt-0 mt-10 justify-between'>
      {
        footerLinks.map((footerLink)=>(
         ...
         <ul className='list-none mt-4'>
          {footerLink.links.map((link,index)=>(
            ...
            text-[16px] leading-[24px] text-white hover:text-secondary
      ...
      border-t-[1px] border-t-[#3f3E45]'>
        <p className='font-poppins font-normal text-center text-[12px] leading-[27px] text-white'>
          Made by Sharika 2023
        </p>
        <div className='flex flex-row md:mt-0 mt-6'>
             {
              socialMedia.map((social,index)=>(
                <img key={social.id}
                src={social.icon} alt='' className={`w-[21px] h-[21px] object-contain cursor-pointer
